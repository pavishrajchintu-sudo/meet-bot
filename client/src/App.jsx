...
function App() {
  ...
  // --- YOUR NEW AWS SERVERLESS ENDPOINT ---
  ...
  const startBot = async () => {
    ...
    setIsRunning(true);
    setSummary("Initializing Scribe AI Bot... 🚀\n\nWaking up AWS Serverless Engine...");
    
    try {
      // Connect directly to the AWS Lambda Function URL
      ...
      if (response.ok) {
        ...
        setSummary(data.error || "Error: Bot failed to execute on AWS.");
        setIsRunning(false);
      }
    } catch (error) {
      console.error("Start Error:", error);
      setSummary("Error: Could not connect to AWS backend.");
      setIsRunning(false);
    }
  };
  ...
  const stopBot = async () => {
    setIsRunning(false);
    setSummary("Synthesizing meeting data... 🧠\n\n*(Note: Full summarization is pending final AWS backend updates. Bot has been disconnected.)*");
    // Removed the /api/stop-bot fetch since AWS doesn't use routing yet.
  };
  ...
  useEffect(() => {
    const poll = setInterval(async () => {
      // Polling is temporarily disabled for AWS Serverless to prevent network errors.
      // AWS Lambda shuts down entirely when not running, so there is no persistent memory to poll.
    }, 4000);
